...
class ArucoNode(Node):
    def __init__(self):
        ...
        self.marker_id_unique = self.create_publisher(Int32, '/MARKER_ID', 10)
    ...
    def compressed_callback(self, compressed_data):
        ...
        if marker_ids is not None:
            for i, marker_id in enumerate(marker_ids.flatten()):
                if marker_id == 0 or marker_id == 3 or marker_id == 192: # those marker id detected 
                    continue
                ...
                #self.markers = int(marker_id)
                self.marker_id_unique.publish(Int32(data=int(marker_id)))
        ...
        cv2.waitKey(1)
        #if self.markers != 0:
            #self.marker_id_unique.publish(Int32(data=self.markers))
