...
public:
  ...
  : rclcpp::Node("patrolling_controller"), state_(STARTING)
  {
      goal_subscriber_ = this->create_subscription<std_msgs::msg::String>(
          "dynamic_goal_topic", 10,
      std::bind(&PatrollingController::goalCallback, this, std::placeholders::_1));
  }

  void goalCallback(const std_msgs::msg::String::SharedPtr msg)
  {
    dynamic_goal_ = msg->data;
    RCLCPP_INFO(this->get_logger(), "Dynamic goal received: %s", dynamic_goal_.c_str());
  }
  void init()
  {
  ...
  void init_knowledge()
  {
	...
	problem_expert_->addPredicate(plansys2::Predicate("(connected wp1 wp4)"));
          ...
          if (!executor_client_->execute_and_check_plan() && executor_client_->getResult()) {
            if (executor_client_->getResult().value().success) {
              ...
              if (executor_client_->start_plan_execution(plan.value())) {
                 state_ = PATROL_WP4;
                // ## I added this line
                // std::cout << "Finished patrolling" << std::endl;
                // break;   
              }
            } else {
              for (const auto & action_feedback : feedback.action_execution_status) {
      ...
      case PATROL_WP4:
        {
          ...
          if (!executor_client_->execute_and_check_plan() && executor_client_->getResult()) {
            if (executor_client_->getResult().value().success) {
              ...
              // Set the goal for next state
              std::string goal_string = "(and(patrolled " + dynamic_goal_ + "))";
              problem_expert_->setGoal(plansys2::Goal(goal_string));
              std::cout << "Smallest IDs is " << dynamic_goal_ << std::endl;
